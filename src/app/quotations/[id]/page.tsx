...
export default async function QuotationDetailPage({
  ...
  // Run auth check and DB query concurrently in parallel
  ...
  const customerEmail = doc.customerEmail || doc.snapshotCustomerEmail || doc.customer?.email || null;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-6" style={{ borderColor: "var(--border)" }}>
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: "var(--muted-foreground)" }}>
            Prepared by {doc.createdBy.name} on{" "}
        ...
        <div className="space-y-4 md:col-span-1">
          ...
          >
            ...
            <div>
              ...
              {customerAddress && <p className="text-sm mt-1 whitespace-pre-line" style={{ color: "var(--muted-foreground)" }}>{customerAddress}</p>}
              {customerPhone && <p className="text-xs mt-2" style={{ color: "var(--muted-foreground)" }}>Phone: {customerPhone}</p>}
              {customerEmail && <p className="text-xs font-mono-nums" style={{ color: "var(--muted-foreground)" }}>Email: {customerEmail}</p>}
              {customerTaxId && <p className="text-xs font-mono-nums mt-1" style={{ color: "var(--muted-foreground)" }}>TRN: {customerTaxId}</p>}
            </div>
          </div>
                ...
                {doc.items.map((item, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 font-semibold font-mono-nums" style={{ color: "var(--accent)" }}>{item.partNumber}</td>
